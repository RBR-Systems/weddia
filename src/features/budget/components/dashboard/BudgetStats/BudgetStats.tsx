...
import { BUDGET_STATUS_AT_RISK_PERCENT, BUDGET_STATUS_OVER_PERCENT, REMAINING_WARNING_RATIO } from "../../../constants/budget.constants";
...
function getRemainingColor(remaining: number, budget: number): string {
  ...
  if (remaining < budget * REMAINING_WARNING_RATIO) return "var(--budget-warning)";
  return "var(--budget-success)";
}

function getSpentProgressColor(pct: number): string {
  if (pct >= BUDGET_STATUS_OVER_PERCENT) return "var(--budget-danger)";
  if (pct >= BUDGET_STATUS_AT_RISK_PERCENT) return "var(--budget-warning)";
