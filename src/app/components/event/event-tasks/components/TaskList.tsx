...
import type { Task, TaskFilters, TaskCategory } from "../types/task.types";
    ...
    result.sort((a, b) => {
      ...
      switch (filters.sortBy) {
        ...
        case "priority": {
          ...
          cmp =
            (pOrder[a.priority] ?? 9) - (pOrder[b.priority] ?? 9);
          break;
        }
        case "created_at":
          cmp =
  ...
  const tableColumns: ColumnsType<Task> = [
    {
      ...
      render: (text: string, record: Task) => (
        <Button type="link" onClick={() => onTaskClick(record)} style={{ padding: 0, height: "auto", whiteSpace: "normal", textAlign: "left" }}>
          {text}
        </Button>
      ),
    },
    ...
    },
    {
      ...
      key: "priority",
      width: 100,
      render: (p: string) => <Tag color={priorityColors[p]}>{t(`tasks.priority.${p}`)}</Tag>,
    },
    {
      ...
      key: "status",
      width: 120,
      render: (s: string) => <Tag color={statusColors[s]}>{t(`tasks.status.${s}`)}</Tag>,
    },
    {
      ...
      render: (_: unknown, record: Task) => (
        <Tag style={{ borderColor: record.category_color, color: record.category_color }}>{record.category_name}</Tag>
      ),
    },
    {
        ...
        <Avatar.Group max={{ count: 3 }} size={24}>
          {record.assignees.map((a, idx) => (
            <Tooltip key={a.assignment_id} title={a.user_name}>
              <Avatar size={24} style={{ backgroundColor: avatarColors[idx % avatarColors.length], fontSize: 11 }}>
    ...
    },
    {
      ...
      width: 80,
      render: (_: unknown, record: Task) => (
        <Progress percent={record.completion_percentage} size="small" strokeColor="#c9a38c" showInfo={false} />
      ),
    },
    {
      title: "",
        ...
        record.status !== "completed" ? (
          <Tooltip title={t("tasks.card.markComplete")}>
            <Button
              ...
              onClick={(e) => { e.stopPropagation(); onQuickComplete(record); }}
            />
          </Tooltip>
        ) : null,
    },
      ...
      <div className={styles.taskListHeader}>
        ...
        <div className={styles.taskListActions}>
          <Segmented
            ...
            options={[
              { value: "cards", icon: <AppstoreOutlined />, label: t("tasks.viewMode.cards") },
              { value: "table", icon: <UnorderedListOutlined />, label: t("tasks.viewMode.table") },
            ]}
            size="middle"
          />
              ...
              items={[
                {
                  ...
                  label: (
                    <span style={{ color: "#22C55E" }}>
                      <CheckCircleFilled />{" "}
                      {t("tasks.groups.completed")} ({completed.length})
                    </span>
                  ),
                  children: (
                    <div className={styles.taskGrid}>
